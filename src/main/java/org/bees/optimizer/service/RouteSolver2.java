...
@Profile("markov2")
public class RouteSolver2 implements Solver {
    ...
    private RouteBuilder2 routeBuilder;
    ...
    private void makeDecision(ArriveDto arriveDto, TrafficDto trafficDto) {
        ...
        makeDecision(arriveDto.getDuration(), car, arriveDto.getPoint(),car.getCapacity() - arriveDto.getCarSum(), ModelConverter.convertTraffic(trafficDto));
    }

    private void makeDecision(double duration, Car car, int currentPoint, long remainCapacity, double[][] trafficJams) {
        GotoDto gotoDto = routeBuilder.makeDecision(duration, car, currentPoint, remainCapacity, trafficJams);
    ...
    private void initIfReady() {
        if (isReadyForInitialize() && routeBuilder == null) {
            routeBuilder = new RouteBuilder2(ModelConverter.convertRoutes(routesDto), ModelConverter.convertPoints(pointsDto), ModelConverter.convertTokenToCar(tokenDto), gamma, maxRecursionNumber);
        }
    }
